...
def train_multi_level(model, model_D1, model_D2, model_number, src_trainloader, trg_trainloader, src_valloader, trg_valloader, criterion, bd_criterion, bce_criterion, \
    ...
    for e in range(init_epoch-1, epochs):
        ...
        for i in range(train_num_steps):
            ...
            trg_images, trg_masks, trg_boundaries = next(trg_train_iter)
            trg_images, trg_masks, trg_boundaries = trg_images.to(device), trg_masks.to(device), trg_boundaries.to(device)
            ...
            h, w = trg_masks.size(1), trg_masks.size(2)
            ...
            loss_D1 = bce_criterion(D1_out, torch.full_like(D1_out, src_label, device=device))
            loss_D2 = bce_criterion(D2_out, torch.full_like(D2_out, src_label, device=device))
            loss_D1 = loss_D1 / 2
            loss_D2 = loss_D2 / 2
            loss_D1.backward(retain_graph=True)
            loss_D2.backward(retain_graph=True)

            cumulative_D_loss += loss_D1.item() + loss_D2.item()


            ## Training with Target
            D1_out = model_D1(F.softmax(trg_logits[-3], dim=1))
            D2_out = model_D2(F.softmax(trg_logits[-2], dim=1))
            loss_D1 = bce_criterion(D1_out, torch.full_like(D1_out, trg_label, device=device))
            loss_D2 = bce_criterion(D2_out, torch.full_like(D2_out, trg_label, device=device))
            loss_D1 = loss_D1 / 2
            loss_D2 = loss_D2 / 2
            loss_D1.backward(retain_graph=True)
            loss_D2.backward()
        ...
        with torch.no_grad():
            for i in range(val_num_steps):
                ...
                val_seg_loss = cumulative_seg_loss / count
                val_adv_loss = cumulative_adv_loss / count
                val_D_loss = cumulative_D_loss / count
                ...
                monitor.update(
                    i + 1,
                    learning_rate=f"{learning_rate:.5f}",
                    val_seg_loss=f"{val_seg_loss:.4f}",
                    val_adv_loss=f"{val_adv_loss:.4f}",
        ...
        val_adv_losses.append(val_adv_loss)
        ...
        val_loss = val_seg_loss + val_adv_loss + val_D_loss
...
def train_single_level(model, model_D2, model_number, src_trainloader, trg_trainloader, src_valloader, trg_valloader, criterion, bd_criterion, bce_criterion, \
    ...
    for e in range(init_epoch-1, epochs):
        ...
        for i in range(train_num_steps):
            ...
            cumulative_D2_loss += loss_D2.item()
        ...
        with torch.no_grad():
            for i in range(val_num_steps):
                ...
                val_seg_loss = cumulative_seg_loss / count
                val_adv2_loss = cumulative_adv2_loss / count
                val_D2_loss = cumulative_D2_loss / count
                ...
                monitor.update(
                    ...
                    learning_rate=f"{learning_rate:.5f}",
                    val_seg_loss=f"{val_seg_loss:.4f}",
                    val_adv2_loss=f"{val_adv2_loss:.4f}",
                    val_D2_loss=f"{val_D2_loss:.4f}",
                    val_mIoU=f"{val_mIoU:.4f}",
                )


        val_seg_losses.append(val_seg_loss)
        val_adv_losses.append(val_adv2_loss)
        val_D_losses.append(val_D2_loss)
        val_mIoUs.append(val_mIoU)

        monitor.stop()


        val_loss = val_seg_loss + val_adv2_loss + val_D2_loss
        if best_val_loss is None or val_loss < best_val_loss:
            save_model(model, f"{res_dir}/weights/best_{model_number}.pt")
            save_model(model_D2, f"{res_dir}/weights/best_D2_{model_number}.pt")
            monitor.log(f"Model saved as best_{model_number}.pt\n")
